    ...
    fun loadFilteredBookings(
        ...
        bookingJob = viewModelScope.launch {
            ...
            try {
                ...
                bookingRepository.getAllBookingsByHotelId(hotelId).collect { allBookings ->
                    ...
                    val filteredList = allBookings.filter { booking ->
                        ...
                        when (filterType) {
                            BookingFilterType.ARRIVALS -> {
                                startDate == targetDate && booking.status == BookingStatus.CONFIRMED && booking.stayStatus == StayStatus.NONE
                            }

                            BookingFilterType.DEPARTURES -> {
                                endDate == targetDate && booking.status == BookingStatus.CONFIRMED && booking.stayStatus == StayStatus.CHECK_IN
                            }
